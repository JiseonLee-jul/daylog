...
**수신 백엔드는 선택사항이다.** Langfuse, MLflow는 현재 권장 도구일 뿐, OTLP를 지원하는 어떤 도구든 교체 가능. 단, span 조회는 OTLP 표준이 아닌 각 백엔드의 고유 API를 사용하며, 이 종속성은 SpanLoader 추상화가 캡슐화한다. 백엔드 교체 시 SpanLoader 구현체만 교체하면 되고, Evaluator/Metric/Group은 무수정이다.
