...
═══════════════════════════════════════════════════════════════════
 ...
 └──────┬───────┘     └──────┬───────┘     │ EntityExt.  ─┘   │
     ...
     각 Component의 __call__ 실행 시, 두 경로가 동시에 갈라짐:
              ...
              ▼                                  ▼
 ┌──────────────────────────┐    ┌──────────────────────────┐
 │  Artifact 저장             │    │  span 자동 방출            │
 │  (파이프라인 기존 저장소)    │    │  (Base Class가 OTel 은닉)  │
 │                          │    │                          │
...
**두 경로의 분기**: Component의 `__call__`이 실행되면 ① 결과물은 기존 저장소에, ② 실행 흔적은 OTel로 **동시에** 나간다. 순차가 아닌 병렬이며, 서로 독립적이다.
**① artifact 경로**: L1이 기존 저장소에 저장한 산출물을 L3의 ArtifactLoader가 읽어 결과물 품질을 평가.
**② span 경로**: L2가 방출한 실행 흔적을 L3의 SpanLoader가 읽어 실행 품질을 평가.
**MLflow Run**: ①②의 평가 결과와 L2의 trace 데이터가 동일한 run에 합류.
