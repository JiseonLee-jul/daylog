    ...
    def __call__(self, input: TInput) -> TOutput:
        with tracer.start_as_current_span(self.component_name) as span:
            ...
            try:
                ...
                span.set_attribute("kms.status", "completed")
                self._set_output_attributes(span, result)
                return result
            except Exception as e:
                span.set_attribute("kms.status", "failed")
...
EleutherAI의 lm-evaluation-harness(12K+ stars)는 이 문제를 **"평가 정의 = 데이터(YAML) + 메트릭(plugin) + 집계(group)"의 3분리**로 해결했다. Registry가 컴포넌트 계약 경계를, YAML 상속이 메트릭 추가 마찰을, Group이 계층적 집계를 해결한다. 대안(pytest fixture, Great Expectations, DAG 기반, Promptfoo)은 이 세 가지를 동시에 충족하지 못한다. 본 설계는 이 패턴을 차용하되, 평가 대상을 LLM 모델이 아닌 **KMS 파이프라인 컴포넌트**에 맞게 재설계한다.
...
kms/eval/
  ...
  preprocess/
    ...
    golden_strip_fm.py         golden 정규화 (frontmatter 제거)

  loggers/
    otel_logger.py          평가 결과를 OTel Metrics로 방출

  run.py                    CLI 진입점 (YAML 읽기 → 실행 → 집계 → 방출)
...
코드 수정 없이 YAML 한 장으로 새 평가를 추가한다. 아래는 구조 예시이며, 필드 명세(applicable_depths, weight_by_size 정의 등)는 추후 상세 설계한다.
...
class ParserEvaluator(Evaluator):
    def evaluate(self, artifact, golden):
        return {
            # 전체 텍스트 비교
            "chrf_plus": chrf_plus.compute(artifact.body_md, golden.body_md),

            # 표만 추출하여 비교
            "teds_table": teds_table.compute(
                extract_tables(artifact.body_md),
                extract_tables(golden.body_md)
            ),

            # 실행 메타 기반 실행 품질
            "latency": p95_latency.compute(artifact.exec_meta),
        }
```
...
| Loader | 읽는 곳 | 용도 |
|---|---|---|
| **ArtifactLoader** | S3, PostgreSQL, Graph DB | 파이프라인 산출물 + 실행 메타 (ID 기반 조회) |
| **GoldenLoader** | golden/ 디렉토리 | 기대 출력물 (사람이 작성) |

실행 품질 메트릭(지연시간, 토큰 비용)은 파이프라인이 artifact와 함께 저장한 실행 메타에서 읽는다. span을 백엔드에서 꺼내오지 않으므로 벤더 종속이 발생하지 않는다. Evaluator 코드는 환경에 무관하게 동일하며, ArtifactLoader 구현(Local/S3)만 교체하면 된다.
...
**수신 백엔드는 선택사항이다.** Langfuse, MLflow는 현재 권장 도구일 뿐, OTLP를 지원하는 어떤 도구든 교체 가능. OTLP는 방출 전용이며, 평가 워커는 백엔드에서 span을 꺼내오지 않는다. 평가에 필요한 데이터는 파이프라인 저장소(S3/DB)에서 직접 읽으므로, 백엔드 교체가 평가 체계에 영향을 주지 않는다.
...
평가 워커는 파이프라인 trace를 이어받지 않는다. 과거 artifact를 **저장소에서 조회**할 뿐이므로 Trace Propagation과 무관하다.
...
1장에서 정의한 R1–R6을, 어떤 설계 결정으로 만족시키는가. 그리고 **왜 그것이 가능한가**.

---

### R1. 컴포넌트 독립성

> 평가 코어(Metric, Group, Registry, Loader)는 컴포넌트 변경과 무관하게 수정하지 않는다. Evaluator는 유일하게 수정이 허용되는 번역 계층이다.

**가능한 이유**: 평가가 컴포넌트 출력을 이해해야 하는 것은 본질적으로 피할 수 없다. 이 "이해"를 Evaluator 한 곳에 국한시킴으로써, 나머지 평가 코어는 컴포넌트 변경과 완전히 격리된다.
...
**R1과 R3의 긴장 관계**: R3에서 "Evaluator 가공 로직은 변경될 수 있다"고 인정한다. 이는 R1의 "무수정"과 표면적으로 충돌하지만, R1의 범위를 **평가 코어**로 한정함으로써 해소된다. 컴포넌트 변경 시 영향 범위는 Evaluator 한 곳으로 국한되며, Metric/Group/Registry/Loader는 건드리지 않는다.

**검증 시나리오**: Parser를 Docling에서 Marker로 교체할 때, `metrics/`, `api/`, `loaders/`, `configs/groups/` 내 변경 파일이 0건이어야 한다. `evaluators/parser.py`의 가공 로직만 수정이 허용된다.
...
**가능한 이유**: Evaluator가 **두 종류의 입력**(artifact + 실행 메타)을 받을 수 있고, 컴포넌트 단위로 분리되어 있어 파이프라인 중간 단계와 최종 그래프를 **같은 구조로** 평가할 수 있기 때문이다.
...
| **두 차원 평가** (3장) | 파이프라인이 artifact와 함께 실행 메타(소요시간, 토큰 수)를 저장하므로, Evaluator는 "결과물이 정확한가"(artifact)와 "과정이 건강한가"(실행 메타)를 같은 ArtifactLoader로 동시에 읽어 측정할 수 있다. |
...
**L2 평가 — Celery와 무관, 파이프라인 저장소에서 직접 읽기:**

| 설계 결정 | 왜 이것이 분산 대응을 가능하게 하는가 |
|---|---|
| **Observe First, Evaluate Async** (3장) | 파이프라인이 artifact + 실행 메타를 저장하고 완료하면, 평가 워커가 별도 프로세스에서 비동기로 실행된다. span을 백엔드에서 꺼내오지 않고 파이프라인 저장소에서 직접 읽으므로, Trace Propagation 문제도 벤더 종속도 발생하지 않는다. |
| **Loader 추상화** (5장) | 로컬에서는 `LocalArtifactLoader`, 분산에서는 `S3ArtifactLoader`. Evaluator 코드는 동일. 환경 차이가 Loader 한 곳에 캡슐화된다. |
...
| **벤더 중립** (7장) | OTLP 방출 전용이므로, 수신 백엔드를 자유롭게 교체·추가 가능. 평가 워커는 백엔드에서 데이터를 꺼내오지 않으므로 벤더 종속이 발생하지 않는다. |
...
## 9.1 이 설계에서 다루지 않는 것 — 필요 없음

(현재 해당 없음)

## 9.2 이 설계에서 다루지 않는 것 — 추후 과제
