...
```
① DATA          corpus/, golden/, artifact(S3/DB), span(File/OTLP)
     │
     ▼
...
```
kms/observability/
  telemetry.py     TracerProvider + MeterProvider 초기화, Exporter 등록
  semconv.py       KMS 속성 규약 상수 정의

kms/pipeline/
  base.py          Component.__call__에서 span 생성 (유일한 L1 계측 지점)

kms_eval/
  loggers/
    otel_logger.py   평가 결과를 OTel Metrics로 방출 (유일한 L2 계측 지점)
```
